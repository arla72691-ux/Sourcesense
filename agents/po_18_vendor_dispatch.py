...
def simulate_vendor_acknowledgment(db_path, po_ref_id):
    ...
    with get_db(db_path) as conn:
        ...
        cursor.execute("INSERT INTO Process_Events_Log (Process_ID, Entity_Type, Entity_ID, Event_Type, Actor, Created_At) VALUES (?,?,?,?,?,?)",
                       ('S2C.PO.19', 'PO', po_ref_id, 'Vendor_Acknowledged', 'System_Sim', datetime.datetime.now().isoformat()))
    return True
